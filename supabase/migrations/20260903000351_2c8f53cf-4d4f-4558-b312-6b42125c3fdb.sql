-- lovable-cron-fallback-reviewed: 1440 runs/day; video generation is an external provider queue (fal.ai Seedance/MiniMax/Veo) with no webhook callback, so periodic polling is needed to progress pending jobs. Idempotent and only touches pending rows.
