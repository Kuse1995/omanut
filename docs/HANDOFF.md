...
### 5b. omanut-harness — external LLM decision layer (NEW, proposed)
...
- **Deploy steps (human)**: farm-deploy.sh omanut-harness 3003 → add-harness.sh →
  Supabase env (OMANUT_HARNESS_URL, OMANUT_HARNESS_API_KEY) → pilot on boss line →
  widen to OmanutBMS.
