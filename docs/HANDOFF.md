...
- **v2.5 — Seedance 2.5 + chat image refs + script passthrough (PRs #25/#26, merged)**: omanut-motion accepts `script_override` (a caller-written full script passes VERBATIM to the model — sub-agents skipped) and `image_urls` (routed to `bytedance/seedance-2.5/reference-to-video` — locks a product/character across up to a 30s take with native dialogue audio; 480p default). agent-console accepts `image_urls`; AgentConsole UI has a 📎 upload (company-media storage — path MUST start with the company uuid, storage policies cast foldername[1]).
- **⚠️ fal queue gotchas (learned the hard way, 2026-09-03)**: (1) fal queue endpoints only accept **POST** for submissions — a POST to ANY queue path (even one that looks like a status route) **creates a new paid generation**; status checks are GET on the canonical `status_url` returned in the submit response (store it: `scheduled_post_data.fal_status_url`), and the result lives at the companion `response_url` (`video.url`). An agent probe accidentally triggered a default render this way (~$2-5 — check fal billing). (2) First live render (Hot Magic 20s script, 480p, ~20s) COMPLETED and the mp4 is on fal's CDN — the platform poller needed the URL-pattern fix above (PR #29) to collect it into storage + WhatsApp.
